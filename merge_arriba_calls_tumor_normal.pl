...
#### Base Arriba output dir for the gene fusion calls
#Replace the path of the "$results_dir" with the path of the "results_dir" set in the arriba_{tumor|normal}.sh 
#The expected results are stored in "$results_dir/$type/$sample/fusions.tsv"
...
for ($i = 0; $i < @samples; $i++) {
    ...
    $input_file = "$results_dir/$type/$sample/fusions.tsv";
